  ...
  var data3y = localStorage.getItem('userToken');
  let myDataxy = JSON.parse(data3y);
  let myToken = myDataxy.access_token;
    ...
    function fineDate(date){
      ...
      $.ajax({
        ...
        success: function(value){
            if(value.data  == 0){
                $('.hideItem').hide();
                $('#noItem').text("You haven't saved any jobs yet.");
            }
           $.each(value.data, function(index,value){
              var showD = fineDate(value.added_date);
              //var showD = moment(value.added_date).fromNow();
               $('#favies').append('<tr id="dynaG'+value.id+'"">'+
               '<td>'+
                   '<div class="manage-candidate-wrap">'+
                       '<h2 class="widget-title pb-1">'+'<a href="/jobs/details?data='+ value.job_offer.slug +'"" class="color-text-2">'+value.job_offer.available_post+'</a>'+'</h2>'+
                       '<p class="mb-2">'+
                           '<span>'+'<i class="la la-map-marker font-size-16">'+'</i>'+value.job_offer.city_of_assignment.en_US+'</span>'+
                       '</p>'+
                   '</div>'+
               '</td>'+
               '<td>'+showD+'</td>'+
               '<td class="text-center">'+
                   '<div class="manage-candidate-wrap">'+
                       '<div class="bread-action pt-0">'+
                           '<ul class="info-list">'+
                               '<li class="d-inline-block">'+'<a href="/jobs/details?data='+ value.job_offer.slug +'"" >'+'<i class="la la-eye" data-toggle="tooltip" data-placement="top" title="View">'+'</i>'+'</a>'+'</li>'+
           ...
           $('#dyna'+value.id).click(function(){
               $.ajax({
                url: `https://jobatrac.com/api/favorites/${value.id}/destroy`,
                type: 'DELETE',
                headers: myHeaders,
                dataType: "json",
                success: function(){
